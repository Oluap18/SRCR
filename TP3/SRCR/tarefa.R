...
test_optimal <- function(e = "geral") {
  if (e == "treino")
    regg1 <- regsubsets(Tarefa ~ KDT + MA + MV + TBC + DDC + DMS + AED + ADMSL, package$treino, nvmax = 10)
  else
    regg1 <- regsubsets(Tarefa ~ KDT + MA + MV + TBC + DDC + DMS + AED + ADMSL, package$exaustao, nvmax = 10)
  print(summary(regg1))
}
...
treina_rede <- function(ninter = c(7,6,4), alg = "rprop+", lr = 0.1) {
  p <- package
  p$nnet <- neuralnet(formula, package$treino, hidden = ninter, rep = 3, lifesign = "full", linear.output = FALSE, stepmax = 20000, threshold = 0.01,algorithm = alg, learningrate = lr)
  return(p)
}
...
showCasosTeste <- function() {
  ...
  p$result <- data.frame(atual = package$teste$Tarefa, previsao = p$nnet_result$net.result)
  ...
  plot(round(p$result$atual * 3), temp, xlab = "Tarefa Correta", ylab = "Tarefa Prevista",
      col = "black", main = "Tipo de Tarefa", pch = 16, cex = .5)
...
#########################################################################################
##############################   IDENTIFICAR AS 3 TAREFAS   #############################
#########################################################################################
##############################   1 - TAREFA "WORK"          #############################
##############################   2 - TAREFA "PROGRAMMING"   #############################
##############################   3 - TAREFA "OFFICE"        #############################
#########################################################################################
...
#Atualizar com as variaveis desejadas
formula <- Tarefa ~  MA + DDC + DMS
teste.01 <- subset(package$teste, select = c("MA","DDC", "DMS"))

#Treinar rede com NosInteriores e Algoritmo
package <- treina_rede(c(10), "rprop+")
